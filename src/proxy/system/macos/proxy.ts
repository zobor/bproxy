...
// 设置网络设备代理状态
export const setNetworkProxyStatus = ({ deviceName, status }: { deviceName: string; status: 'off' | 'on' }) => {
...
export const getNetworkHttpProxyInfo = (deviceName: string) => {
  return Object.fromEntries(
    spawnSync('networksetup', ['-getwebproxy', deviceName])
      .stdout.toString()
      .split('\n')
      .filter((item) => !!item)
      .map((item) => {
...
export const getNetworkHttpsProxyInfo = (deviceName: string) => {
  return Object.fromEntries(
    spawnSync('networksetup', ['-getsecurewebproxy', deviceName])
      .stdout.toString()
      .split('\n')
      .filter((item) => !!item)
      .map((item) => {
