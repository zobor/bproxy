...
class RulePattern {
  rulesPattern() {
    ...
    if (options.matched) {
      'file|path|status|jsonp|response'.split('|').forEach((item) => {
        if (options.disableHttpRequest) return;
        if (options.rule && Object.hasOwnProperty.call(options.rule, item)) {
  ...
  readLocalData() {
    ...
    if (options.rule.file) {
      const stat = fs.existsSync(options.rule.file);
      if (stat) {
        this.dataset.httpStatus = 200;
        this.writeHead(this.dataset.httpStatus, this.dataset.responseHeaders);
        const readStream = fs.createReadStream(options.rule.file);
      ...
      const stat = fs.existsSync(options.rule.jsonp);
      if (stat) {
        const readStream = new Readable();
        let body = fs.readFileSync(options.rule.jsonp, 'utf-8');
        body = body.replace(/^[^(]+\(/, '').replace(/\)$/, '');
        const callback = options.rule.callbackParameter || 'callback';
      ...
      const filepath = options.rule.path + options.filepath;
      ...
      options.rule.response(
        this.options.url,
        this.dataset.responseHeaders,
        this.dataset.res, request,
      );
    }
  }
